...
###################################################################################################
#Drop first 4 days: Initial Rolling Window
data <- data %>% 
...
#Fit model to training data
rf.best <- randomForest(y~. , data=train_2, mtry = 19, ntree=500, nodesize = 25,importance = TRUE, metric= 'Accuracy', na.action = na.omit )
#Out of sample prediction
...
#Fit model to training data
rf.best <- randomForest(y~. , data=train_3, mtry = 19, ntree=500, nodesize = 25,importance = TRUE, metric= 'Accuracy', na.action = na.omit )
...
RF_P <- mean(c(precision_1,precision_2,precision_3))
RF_R <- mean(c(recall_1,recall_2,recall_3))
RF_F1 <- mean(c(F1_1,F1_2,F1_3))
